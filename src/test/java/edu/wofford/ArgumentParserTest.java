...
public class ArgumentParserTest {
  ...
  @Test
  public void testParseCreatedXML() {
    for (int i = 0; i < argumentNames.length; i++) {
      Argument arg = new Argument(argumentNames[i]);
      arg.setType("float");
      parser.setArgument(arg);
    ...
    File file = new File("stripped_tmp.xml");
    String[] argumentValues = { "7", "5", "-t", "ellipsoid", "2" };
    receiver.parseXML(file.getAbsolutePath());
    receiver.setArgumentValues(argumentValues);
    assertEquals("7", receiver.getValue("length"));
  ...
  public void testDifferentSetsOfMutuallyExclusiveArgsError() {
    try {
      String[] argumentValues = { "7", "5", "2", "--color", "blue", "--shape", "circle", "--hue", "red", "--test1",
          "1" };
